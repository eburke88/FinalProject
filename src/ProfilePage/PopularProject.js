    ...
    <Link to={link}>  {/* link to unique link */}
      <div onClick={() => {window.scrollTo({top: 0, left: 0, behavior: 'smooth'});}} className="song" key={props.id} onMouseEnter={()=> setHover(true)} onMouseLeave={()=> setHover(false)}>
        <div className="song-info">
            ...
            <img className={`play-button ${hover ? "Show" : ""}`} src="./FinalProject/content/Images-Profile/playbuttonsmall.png" alt="a pink play button"/> {/* show play button on hover */}
            <img className="popular-image" src={"./FinalProject/content/" + props.src} alt={props.alt}/>
